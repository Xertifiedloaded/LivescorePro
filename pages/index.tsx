...
export default function HomePage() {
  ...
  const heroStats = {
    totalMatches:
      (todayMatches?.data?.matches?.length || 0) + (liveMatches?.data?.matches?.length || 0),
        ...
        <div className="container mx-auto px-6 relative">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-yellow-500/20 text-yellow-300 rounded-full px-5 py-2 mb-8 border border-yellow-500/30 backdrop-blur-sm">
              ...
              <span className="text-sm font-semibold">Live Scores & Smart Betting Tips</span>
            </div>
            ...
            <p className="text-sm text-slate-300 mb-12 max-w-3xl mx-auto leading-relaxed">
              Stay ahead with real-time football scores, expert match predictions, and daily betting
              tips — all in one sleek and reliable platform.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              ...
              <div className="bg-slate-800/60 backdrop-blur-sm rounded-2xl p-8 border border-slate-700/50 shadow-xl">
                <div className="text-4xl font-bold text-purple-400 mb-2">{heroStats.leagues}</div>
                <div className="text-slate-400 text-sm font-medium uppercase tracking-wider">
                  Premium Leagues
                </div>
              </div>
        ...
        <section className="mb-24">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <h2 className="text-4xl font-bold text-white mb-2">Today's Fixtures</h2>
              <p className="text-slate-400 text-lg">
          ...
          {todayMatches?.data?.matches && todayMatches.data.matches.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="text-center py-20 bg-slate-800/40 backdrop-blur-sm rounded-2xl border border-slate-700/50">
              ...
              <h3 className="text-xl font-semibold text-white mb-3">No matches today</h3>
              <p className="text-slate-400">Check back tomorrow for new fixtures and predictions</p>
            </div>
          )}
        </section>
        ...
        <section className="mb-24">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <h2 className="text-4xl font-bold text-white mb-2">Elite Leagues</h2>
              <p className="text-slate-400 text-lg">
          ...
          {popularMatches?.data?.leagues && popularMatches.data.leagues.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {popularMatches.data.leagues.slice(0, 6).map((leagueData: LeagueWithMatches) => (
                <LeagueCard key={leagueData.league.code} leagueData={leagueData} />
              ))}
            </div>
          ) : (
            <div className="text-center py-20 bg-slate-800/40 backdrop-blur-sm rounded-2xl border border-slate-700/50">
              <Trophy className="h-16 w-16 text-slate-600 mx-auto mb-6" />
              <p className="text-slate-400 text-lg">Loading premium leagues...</p>
            </div>
          )}
        </section>
        ...
        <section className="py-20 bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-sm rounded-3xl border border-slate-700/30 mb-24">
          <div className="px-8 md:px-16">
            <div className="text-center mb-16">
              <h2 className="text-2xl  font-bold text-white mb-6">Professional-Grade Features</h2>
              <p className="text-slate-300 text-xs lg:text-sm max-w-3xl mx-auto leading-relaxed">
                Advanced tools and insights designed for serious football betting professionals and
                enthusiasts
              </p>
            </div>
        ...
        <section className="text-center py-16 sm:py-20 bg-gradient-to-r from-blue-600 via-purple-600 to-cyan-600 rounded-3xl text-white relative overflow-hidden">
          ...
          <div className="relative max-w-4xl mx-auto px-4 sm:px-8">
            ...
            <p className="text-sm sm:text-xl text-blue-100 mb-8 sm:mb-10 leading-relaxed max-w-3xl mx-auto">
              Join the elite circle of professional bettors who rely on BetFootball Pro for
              data-driven predictions. Start your premium trial today—no commitment required.
            </p>
...
function FeatureCard({
  ...
  return (
    <div className="text-left group hover:transform hover:scale-105 transition-all duration-500">
      ...
      <p className="text-sm sm:text-sm text-slate-300 leading-relaxed">{description}</p>
    </div>
  )
}
    ...
    <Card className="group hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-500 border border-slate-700/50 shadow-xl overflow-hidden bg-slate-800/60 backdrop-blur-sm hover:border-blue-500/30">
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <span className="text-slate-400 text-xs sm:text-sm font-medium">Upcoming matches</span>
          <Badge
            variant="secondary"
    ...
    <Card className="group hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-500 border border-slate-700/50 shadow-xl overflow-hidden bg-slate-800/60 backdrop-blur-sm hover:border-blue-500/30">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          >
            {isLive && <div className="w-2 h-2 bg-red-400 rounded-full animate-pulse mr-2" />}
            {isLive ? 'LIVE' : match.status}
          </Badge>
          <span className="text-xs sm:text-sm text-slate-400 font-medium">{match.league_name}</span>
        </div>
      </CardHeader>
      <CardContent className="p-4 sm:p-6">
        <div className="space-y-4 sm:space-y-5">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-white text-base sm:text-lg">{match.home_team}</span>
          ...
          <div className="flex items-center justify-between">
            <span className="font-semibold text-white text-base sm:text-lg">{match.away_team}</span>
