...
export default function MatchDetailsPage() {
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      const matchId =
        typeof match.id === 'string' ? Number.parseInt(match.id, 10) : Number(match.id)
      ...
      if (error?.response?.status === 400) {
        if (error?.response?.data?.error === 'You already have a prediction for this match') {
          errorMessage =
            'You have already placed a bet on this match. Check your existing prediction below.'
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              {error instanceof Error
                ? error.message
                : "We couldn't load the match details. Please try again."}
            </p>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-center py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-sm font-semibold shadow">
                        VS
                      </div>
                      <div className="w-12 h-px bg-gray-300" />
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                    <MatchInfoItem
                      ...
                      label="Match Date"
                      value={formatMatchDate(match.match_date)}
                    />
                    {match.venue && (
                      <MatchInfoItem
                    ...
                    {match.prediction_count && (
                      <MatchInfoItem
                        ...
                        label="Predictions"
                        value={`${match.prediction_count} bets placed`}
                      />
                    )}
                  </div>
                </div>
              ...
              <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-md">
                ...
                <CardContent className="px-4 pb-4 sm:px-6">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    />
                    {/* Draw */}
                    <OddsBox label="Draw" value={match.odds_draw} color="yellow" team="Tie Game" />
                    {/* Away */}
                    <OddsBox
                      label="Away"
                ...
                <CardContent className="space-y-5">
                  <div className="bg-purple-50 p-4 rounded-xl space-y-4">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <Badge
                        className={`${getPredictionStatusColor(existingPrediction.status)} border`}
                      >
                    ...
                    <div className="space-y-2 text-sm">
                      <InfoRow
                        label="Bet Type"
                        value={getPredictionTypeDisplay(existingPrediction.prediction_type).label}
                        color={getPredictionTypeDisplay(existingPrediction.prediction_type).color}
                      />
                      <InfoRow
                        label="Stake Amount"
                        value={`$${Number(existingPrediction.stake_amount).toFixed(2)}`}
                      />
                      <InfoRow
                        label="Potential Winnings"
                        value={`$${Number(existingPrediction.potential_winnings).toFixed(2)}`}
                        highlight="green"
                      />
                      <InfoRow
                        label="Total Return"
                        value={`$${(
                          Number(existingPrediction.potential_winnings) +
                          Number(existingPrediction.stake_amount)
                        ).toFixed(2)}`}
                        bold
                        highlight="purple"
                      />
                    ...
                    {stakeAmount &&
                      ...
                      Number.parseFloat(stakeAmount) <= 10000 && (
                        <div className="p-4 bg-green-50 border border-green-200 rounded-xl">
                          <InfoRow
                            label="Potential Winnings"
                            value={`$${calculatePotentialWinnings().toFixed(2)}`}
                            highlight="green"
                          />
                          <InfoRow
                            label="Total Return"
              ...
              <Card className="bg-white shadow-xl border border-blue-200 rounded-2xl">
                ...
                <CardContent className="space-y-4 text-center">
                  <p className="text-gray-600">
                    Create an account or login to start placing your bets.
                  </p>
