-- Add new columns to existing tables to support the enhanced API data
...
-- Add teams table with more details
...
-- Update leagues table with more fields
...
-- Update matches table with more fields
...
ALTER TABLE matches ADD COLUMN IF NOT EXISTS home_team_id INTEGER REFERENCES teams(id);
ALTER TABLE matches ADD COLUMN IF NOT EXISTS away_team_id INTEGER REFERENCES teams(id);
