...
router.post("/login", [body("username").notEmpty().trim(), body("password").notEmpty()], async (req, res) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() })
...
router.post("/refresh", async (req, res) => {
  try {
    ...
    const tokenResult = await pool.query("SELECT user_id FROM refresh_tokens WHERE token = $1 AND expires_at > NOW()", [
      refreshToken,
    ])
    ...
    const userResult = await pool.query("SELECT id, username, email, is_active FROM users WHERE id = $1", [
      decoded.userId,
    ])
